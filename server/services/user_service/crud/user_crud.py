...
async def get_user(id: int, db: AsyncSession) -> UserOut:
    try:
...
async def get_all_users(db: AsyncSession, page: int = 1, size: int = 10)-> UserAll:
    try:
        offset = (page - 1) * size
...
async def update_user(id: int, payload: UserUpdate, db: AsyncSession) -> UserOut:
    try:
...
async def delete_user(id: UserID, db: AsyncSession) -> UserID:
    try:
        result = (await db.execute(
            select(UserModel).where(UserModel.id == id)
        )).scalar_one_or_none()
